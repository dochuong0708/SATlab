...
def Giai_SUDOKU(Bang):
    ...
    counter = [730]
    def dung1_SequencetialCounter(daysodung):
        n = len(daysodung)
        if n <= 1:
            if n==1: Giai.add_clause(daysodung)
            return
        Giai.add_clause(daysodung)
        giatri_S = []
        for _ in range(n-1):
            giatri_S.append(counter[0])
            counter[0] += 1
        Giai.add_clause([-daysodung[0],giatri_S[0]])

        for i in range(1, n-1):
            Giai.add_clause([-daysodung[i],giatri_S[i]])
            Giai.add_clause([-giatri_S[i-1],giatri_S[i]])

            Giai.add_clause([-giatri_S[i-1],-daysodung[i]])
        Giai.add_clause([-giatri_S[n-2],-daysodung[n-1]])
    ...
    for hang in range(9):
            for cot in range(9):
                dung1_SequencetialCounter([laydiachi(hang,cot,giatri) for giatri in range(9)])
        
    for hang in range(9):
            for giatri in range(9):
                dung1_SequencetialCounter([laydiachi(hang,cot,giatri) for cot in range(9)])
    for cot in range(9):
            for giatri in range(9):
                dung1_SequencetialCounter([laydiachi(hang,cot,giatri) for hang in range(9)])
    ...
    for hang_cua_khoi in range(3):
            for cot_cua_khoi in range(3):
                for giatri in range(9):
                    ...
                    dung1_SequencetialCounter(giatri_cua_khoi)
    if Giai.solve():
